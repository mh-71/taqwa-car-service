...
cleanup() {
  ...
  if [ "$SEEDED" = "1" ]; then
    ...
    left=$(d1 "SELECT (SELECT count(*) FROM services WHERE id LIKE 'SRV-9%')
                    ...
                    + (SELECT count(*) FROM vehicles WHERE id LIKE 'VEH-9%') AS n" \
...
COUNTS=$(d1 "SELECT (SELECT count(*) FROM services) + (SELECT count(*) FROM customers) + (SELECT count(*) FROM vehicles) AS n" \
         | grep -oE '"n": *[0-9]+' | grep -oE '[0-9]+')
if [ "${COUNTS:-x}" != "0" ]; then
  cat >&2 <<MSG
REFUSED: services/customers/vehicles already hold ${COUNTS:-?} row(s).
...
echo "  6 services, 2 customers, 2 vehicles"
