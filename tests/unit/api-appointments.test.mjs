...
// C-3 gave appointments writes, so the methods it still refuses are fewer
// and the Allow header names the new ones. The writes themselves are covered
// in tests/unit/api-appointments-write.test.mjs.
for (const m of ['PUT', 'DELETE', 'PATCH']) {
  const res = await call('/api/appointments', { DB: stubDB({ rows: [] }) }, { method: m });
  ok_(`${m} -> 405`, res.status === 405, `got ${res.status}`);
}
{
  const res = await call('/api/appointments', { DB: stubDB({ rows: [] }) }, { method: 'PATCH' });
...
// C-3 gave appointments writes; PUT and DELETE are real routes here now.
