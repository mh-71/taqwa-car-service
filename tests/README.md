...
| `api-security.test.mjs` | The headers and the session edges — that all ten response classes (200/401/404/405/422/503/204, public and protected alike) carry `no-store`, `nosniff`, `Referrer-Policy` and a frame refusal; that the app's CSP allows its one inline script **by hash** with no `'unsafe-inline'`, that the hash actually matches the script on every page, and that nothing uses an inline handler the hash would not cover; that eight malformed or foreign-signed cookies are each refused while the real one is not, that no ordering of a duplicate cookie turns a forgery into access, and that a replayed valid cookie IS accepted until it expires — the documented limitation, asserted so it cannot be quietly assumed away |
| `config.test.mjs` | The deployment configuration — that the DEVELOPMENT binding never holds a real database id (a bare `wrangler deploy` reaching production is the mistake this exists to catch), that the two environments keep distinct Worker names, that no secret value is committable and `.env*`/`.dev.vars*` are ignored **and** unpublishable, that `.assetsignore` still excludes everything but the app, that no npm script deploys or uses `--remote`, and that `DEPLOYMENT.md` covers every provisioning step without claiming the C-12 limitations are solved. It runs `tools/config-check.mjs` as part of the suite, offline and touching no Cloudflare resource |
