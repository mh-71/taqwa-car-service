...
**Nothing in this repository has been deployed.** No production D1 database
exists, no production secret has been set, and no Cloudflare resource has been
created. This file is the checklist for doing that when the time comes — every
step is a command a person runs deliberately, not something the repository does
on its own.

Read it in order. Steps 1–4 create things; step 5 is the first one that makes
the app reachable.
...
| D1 | `taqwa-local` / `local-development-only` | `taqwa-prod` / *placeholder until step 1* |
...
- A **bare `wrangler deploy` cannot touch production.** It carries the
  ...
  `taqwa-api`.
- `--env production` **fails until step 1 is done**, because the database id is
  still the placeholder. That is the intended behaviour, not a bug to route
  around.
...
## 4. Rate-limit the sign-in route
